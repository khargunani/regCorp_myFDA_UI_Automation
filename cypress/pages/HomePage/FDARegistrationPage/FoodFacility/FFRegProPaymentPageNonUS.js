const Locators = {
    ...
    CheckCheckbox: `input[value='wire']`,
    ...
    ViewInvoice: `div[id='bankwireinfo'] button[class='uk-button uk-button-small uk-button-primary uk-margin-left']`,
    Next: `form[name='swiftForm'] div button[type='submit']`,
    Agreement: `(//input[@name='agreeTermsCheckbox'])[2]`,
    Agreement2: `(//input[@name='agreeAutoRenewPlanCheckbox'])[2]`,
    UndersignName: `form[name='swiftForm'] fieldset div div input[name='undersignedName']`,
    UndersignTitle: `form[name='swiftForm'] fieldset div div input[name='undersignedTitle']`,
    UndersignEmail: `form[name='swiftForm'] fieldset div div input[name='undersignedEmail']`,
...
class FFRegProPaymentPageNonUS {
    ...
    fillDetailsForCheckPayment(PaymentDetails) {
        cy.get(Locators.CheckCheckbox).check();
        cy.xpath(Locators.Agreement).check({force:true});
        cy.xpath(Locators.Agreement2).check({force:true});
        cy.get(Locators.UndersignName).type(PaymentDetails.Name);
    ...
    verifyInvoiceDetailsForBankWire() {
        ...
        cy.task('readPdf', 'cypress/downloads/Invoice_for_MCWH2018_().pdf').then((data) => {
            cy.log(data.text)
            cy.log(this.txt);
            cy.log(data.text); // Check if data.text has the expected value
expect(data.text).to.contain(this.txt);
    ...
    configureEmail(PaymentDetails) {
        ...
        cy.wait(2000)
        cy.get(Locators.emailList).contains(PaymentDetails.EmailSubjectForNonUS);
    }
...
export default FFRegProPaymentPageNonUS;